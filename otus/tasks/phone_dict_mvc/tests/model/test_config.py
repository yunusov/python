...
from src.exceptions import ConfigNotFoundException, PhoneDictException
...
@pytest.fixture
def config_wrong():
    with patch("src.model.Config.get_config_name", 
               return_value = "config_wrong.yaml"):
...
def test_storage_folder_wrong(config_wrong):
    try:
        ...
        logger.info(f"{e = }")
        assert type(e) == PhoneDictException
...
def test_open_notfound_file():
    try:
        ...
        logger.info(f"{e = }")   
        assert type(e) == ConfigNotFoundException
