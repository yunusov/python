...
folder_test_homework_0X = current_file.parent.parent
homework_0X = folder_test_homework_0X.name
homework_0X_path = folder_test_homework_0X.parent / homework_0X
dockerfile_path = homework_0X_path / "Dockerfile"

if not (dockerfile_path.is_file() and len(dockerfile_path.read_text().splitlines()) > 5):
    pytestmark = pytest.mark.skip("Dockerfile is not ready " + dockerfile_path._str)
